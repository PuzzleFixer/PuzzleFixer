...
Shader "Hidden/Kvant/Spray/Kernel"
{
    ...
    float4 new_particle_position(float2 uv)	// uv: get from vertex shader, indicate current coordinate
    {
        float t = _Config.w;	// time

        // Random position
        float3 p = float3(nrand(uv, t), nrand(uv, t + 1), nrand(uv, t + 2));
        p = (p - (float3)0.5) * _EmitterSize + _EmitterPos;	// _EmitterSize is emitter->size in editor
															// _EmitterPos is the emitter->center in editor
    ...
    float4 frag_init_position(v2f_img i) : SV_Target	// may roughly think that this shader is a fragment shader,
														// which means these functions list from here all executed
														// after vertex shader, hence v2f_img, as predefined in Pass
														// #pragma vertex vert_img, pass the position of vertex  
														// that used in current fragment
														// SV_TARGET is a system value, means the output will be sent
														// to the next stage, the OutPut Merger;
    {
    ...
    {
        float4 p = tex2D(_PositionBuffer, i.uv);	// tex2D gets a color value from the texture, return rgba
        float3 v = tex2D(_VelocityBuffer, i.uv).xyz;

        // Decaying
        float dt = _Config.z;	// time
        p.w -= lerp(_LifeParams.x, _LifeParams.y, nrand(i.uv, 12)) * dt;

        if (p.w > -0.5)
        {
            // Applying the velocity, update exist position
            p.xyz += v * dt;
            return p;
        }
        else
        {
            // Respawn, get new position
    ...
    SubShader
    {
        ...
        {
            CGPROGRAM
            #pragma target 3.0			// version of compilation, similar to version of HLSL
            #pragma vertex vert_img		// vert_img is a minimal vertex shader that is defined in UnityCG.cginc
										// Its purpose is to forward the vertex data to the fragment shader
